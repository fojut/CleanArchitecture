package org.fojut.sample.presentation.main.presenter;

import org.fojut.sample.data.download.task.DownloadTask;
import org.fojut.sample.presentation.base.internal.di.scope.PerActivity;
import org.fojut.sample.presentation.base.presenter.BasePresenter;
import org.fojut.sample.presentation.base.view.render.LoadView;
import org.fojut.sample.presentation.download.manager.DownloadManager;

import javax.inject.Inject;

/**
 * Created by fojut on 2016/5/13.
 */
@PerActivity
public class SettingPresenter extends BasePresenter<SettingPresenter.View> {

    private DownloadManager downloadManager;

    String downloadUrl = "http://p.gdown.baidu.";
    String downloadUrl2 = "http://p.gdown.baidu.";
    String downloadUrl3 = "http://p.gdown.baidu.";

    public String TASK_NAME_1 = "osbuild.apk";
    public String TASK_NAME_2 = "163.apk";
    public String TASK_NAME_3 = "FM.apk";

    @Inject
    public SettingPresenter(DownloadManager downloadManager) {
        this.downloadManager = downloadManager;
    }

    public DownloadManager getDownloadManager() {
        return downloadManager;
    }

    @Override
    public void setView(View view) {
        super.setView(view);
        downloadManager.addView((DownloadManager.View) view);
    }

    @Override
    public void onResume() {

    }

    @Override
    public void onPause() {

    }

    @Override
    public void onDestroy() {
        downloadManager.removeView((DownloadManager.View) getView());
        setView(null);
    }


    /**
     * Download action
     */
    public void download(String viewKey, DownloadTask.ProgressListener progressListener){
        DownloadManager.DownloadSubscriber downloadSubscriber = downloadManager.newDownloadSubscriber(viewKey, new DownloadTask(downloadUrl, TASK_NAME_1));
        downloadSubscriber.getDownloadTask().addProgressListener(progressListener);
        downloadManager.getDownloadUseCase().execute(downloadSubscriber, downloadSubscriber.getDownloadTask());
    }

    public void pauseDownloadTask(){
        downloadManager.pauseDownloadTask(downloadUrl);
    }

    public void resumeDownloadTask(){
        downloadManager.resumeDownloadTask(downloadUrl);
    }

    /**
     * Download action 2
     */
    public void download2(String viewKey, DownloadTask.ProgressListener progressListener){
        DownloadManager.DownloadSubscriber downloadSubscriber2 = downloadManager.newDownloadSubscriber(viewKey, new DownloadTask(downloadUrl2, TASK_NAME_2));
        downloadSubscriber2.getDownloadTask().addProgressListener(progressListener);
        downloadManager.getDownloadUseCase().execute(downloadSubscriber2, downloadSubscriber2.getDownloadTask());
    }

    public void pauseDownloadTask2(){
        downloadManager.pauseDownloadTask(downloadUrl2);
    }

    public void resumeDownloadTask2(){
        downloadManager.resumeDownloadTask(downloadUrl2);
    }

    /**
     * Download action 3
     */
    public void download3(String viewKey, DownloadTask.ProgressListener progressListener){
        DownloadManager.DownloadSubscriber downloadSubscriber3 = downloadManager.newDownloadSubscriber(viewKey, new DownloadTask(downloadUrl3, TASK_NAME_3));
        downloadSubscriber3.getDownloadTask().addProgressListener(progressListener);
        downloadManager.getDownloadUseCase().execute(downloadSubscriber3, downloadSubscriber3.getDownloadTask());
    }

    public void pauseDownloadTask3(){
        downloadManager.pauseDownloadTask(downloadUrl3);
    }

    public void resumeDownloadTask3(){
        downloadManager.resumeDownloadTask(downloadUrl3);
    }
    public void addDownloadTask3Listener(DownloadTask.ProgressListener progressListener){
        downloadManager.addProgressListener4TaskByUrl(downloadUrl3, progressListener);
    }

    public interface View extends LoadView {
    }

}
